...
class CreatePrimitive(bpy.types.Operator):
    ...
    def modal(self, context, event):
        context.area.header_text_set(self.iloscVertow)
        ...
        if event.type == 'P' and not self.dodalem:
        ...
        if event.type in self.numbers and event.value == 'RELEASE' and self.dodalem:
            ...
            zmienNaIlosc = int(self.iloscVertow)
            context.area.header_text_set(self.iloscVertow)
            if self.dodalem == 'Cylinder':
                bpy.ops.mesh.primitive_cylinder_add(vertices = zmienNaIlosc)
            if self.dodalem == 'Sphere':
                bpy.ops.mesh.primitive_uv_sphere_add(segments = zmienNaIlosc, ring_count = zmienNaIlosc)
        ...
        if event.type == 'MINUS' and self.dodalem and event.value != 'RELEASE':
            if self.iloscVertow == '':
                self.iloscVertow = '1'
            self.iloscVertow = str(int(self.iloscVertow) - 1)
