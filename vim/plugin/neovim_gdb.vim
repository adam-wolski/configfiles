...
command! GdbDebugStart call s:Spawn(0, s:run_gdb(), 0, 0) | call s:BindKeys()
command! GdbDebugStartWithServer call s:Spawn(printf('gdbserver localhost:%d %s', s:gdb_port, g:gdb_file), s:run_gdb(), printf('localhost:%d', s:gdb_port), 0) | call s:BindKeys()
